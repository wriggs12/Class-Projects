...
Test(sfmm_student_suite, collate_up, .timeout = TEST_TIMEOUT) {
	void *x = sf_malloc(sizeof(int));
...
Test(sfmm_student_suite, collate_down_not_wilderness, .timeout = TEST_TIMEOUT) {
	void *x = sf_malloc(sizeof(int));
	void *y = sf_malloc(sizeof(int));
	sf_malloc(sizeof(int));
...
Test(sfmm_student_suite, collate_down_wilderness, .timeout = TEST_TIMEOUT) {
	void *x = sf_malloc(sizeof(int));
...
Test(sfmm_student_suite, collate_down_empty_wilderness, .timeout = TEST_TIMEOUT) {
	void *x = sf_malloc(4096 - 48 - sizeof(sf_header) - sizeof(sf_footer));
...
Test(sfmm_student_suite, realloc_full_block_larger, .timeout = TEST_TIMEOUT) {
	void *x = sf_malloc(16384 - 48 - sizeof(sf_header) - sizeof(sf_footer));
	assert_free_block_count(0, 0);
	
	x = sf_realloc(x, 16384);
...
Test(sfmm_student_suite, realloc_full_block_smaller, .timeout = TEST_TIMEOUT) {
	void *x = sf_malloc(16384 - 48 - sizeof(sf_header) - sizeof(sf_footer));
	assert_free_block_count(0, 0);

	x = sf_realloc(x, 15000);
